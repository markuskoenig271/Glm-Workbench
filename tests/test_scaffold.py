...
from pricing_engine import data, diagnostics, glm, prediction, preprocessing, report
...
def test_stubs_fail_loudly(sample_portfolio: pd.DataFrame) -> None:
    ...
    with pytest.raises(NotImplementedError):
        preprocessing.log_transform(sample_portfolio, ["VehicleAge"])
    with pytest.raises(NotImplementedError):
        glm.fit_frequency_glm(sample_portfolio, "Claims ~ Region")
    with pytest.raises(NotImplementedError):
        glm.fit_severity_glm(sample_portfolio, "ClaimAmount ~ Region")
    with pytest.raises(NotImplementedError):
        prediction.predict_frequency(None, sample_portfolio)
    with pytest.raises(NotImplementedError):
        prediction.predict_pure_premium(None, None, sample_portfolio)
    with pytest.raises(NotImplementedError):
        diagnostics.coefficient_table(None)
    with pytest.raises(NotImplementedError):
        diagnostics.residuals(None)
    with pytest.raises(NotImplementedError):
        diagnostics.information_criteria(None)
    with pytest.raises(NotImplementedError):
